...
def stim_creation(nstim):
    ...
    range_of_dec = np.arange(-0.7, 0.8, 0.1)
    range_of_dec = np.round(range_of_dec, decimals = 1)


    orient_matrix = np.array([])

    for idec in range_of_dec:
        sel_trials = np.where((decision_var_T > idec-0.05) & (decision_var_T < idec+0.05))  # select a matrix with decisions in groups of difficult
        sel = orientations[sel_trials[0]][0:ntrials_per_dec,]
        orient_matrix = np.vstack([orient_matrix, sel]) if orient_matrix.size else sel # concatenate matrixes
